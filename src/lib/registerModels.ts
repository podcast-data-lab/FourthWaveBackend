...
export function parseEpisodeData(episodeData: { [index: string]: any }, podcast: Podcast): EpisodeModelInput {
    let episodeObject: EpisodeObject = {
        ...
        description: getContentType(episodeData?.content, 'text/plain') ?? episodeData?.subtitle.trim() ?? '',
        summary: episodeData?.summary.trim() ?? episodeData.summary_detail.trim(),
        htmlDescription: getContentType(episodeData?.content, 'text/html'),
