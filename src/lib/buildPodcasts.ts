...
export async function registerPodcast(_podcast, totalNo, currentNo) {
    ...
    for (let _episode of _podcast.items) {
        ...
        let episode = await EpisodeModel.findOne({
            slug: `${podcast.slug}/${slugify(
                _episode.title ?? _episode?.itunes?.season + '-' + _episode?.itunes?.episode ?? _episode?.link,
            )}`,
        })
        if (!episode) {
        ...
        console.log(
            `· ${chalk.hex('#D0CFCF')(((epCurrent / epTotal) * 100).toFixed(2) + `%`)} - ${chalk.hex('#86BBD8')(
                episode.title,
            )} saved. `,
        )
    }
